<template>
    <page-with-filters 
        ...
        contentStyle="background-color: #fff;box-shadow: 0 0 5px rgba(221, 221, 221, .2);"
        @submit="handleSubmit" 
        ...
        <template v-slot:button-circle>
            <button-circle v-show="formName" v-can="permissionName" @click="pushRouter(props.formName)"/> 
        </template>
...
import { interpoleData, dynamicSort } from "./utils.js"
...
watch(() => $route.name, () => setTimeout(handleResetFilters, 120))
...
const stateRows = computed(() => {
    ...
    return interpoleData(props.cells, sortedData || state.data)
})
</script>