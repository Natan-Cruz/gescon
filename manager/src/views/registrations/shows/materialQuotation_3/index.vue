<template>
    <form-page-without-script
        ...
        <div class="tree-table" style="margin-bottom: 32px">
            <div class="content-tree-table">
                <item-table-header :data="tableHeader" :numberOfColumns="numberOfColumns" :showCheckbox="false" style="font-weight: bold"/>
                <ul>
                    <item-table v-for="(d,i) of data" :key="i" :data="d" :numberOfColumns="numberOfColumns" :showCheckbox="true"
...
import itemTable from "./items/itemTable.vue"
import itemTableHeader from "./items/itemTableHeader.vue"
...
export default {
    components: { formPageWithoutScript, selectedItem, itemTable, itemTableHeader },
    setup(){
        ...
        const data = computed(() => {
                ...
                materialNames.forEach( material_name => {
                    ...
                    if(item){
                        obj = {
                            ...
                            unity: item.unity,
                            total: item.quantity * item.unitary_value
                        }
                    }
                    if(has){
                        has.push(obj)
            ...
            return Array.from(map).map(([ key, value ]) => {
                ...
                let item = value.find( elem => typeof elem !== "undefined")
                item = item || {};
