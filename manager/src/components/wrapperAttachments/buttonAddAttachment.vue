<template>
    <div class="wrapper-button-add-attachment">
        ...
        <input type="file" :id="uuid" @input="$emit('event-load-file', $event)" multiple="multiple" style="display:none" :disabled="disabled">
    </div>
</template>

<script>
export default {
    name: "button-add-attachment",
    props: { disabled:  Boolean },
    setup(){
        return {
            uuid: 'input-attachment_' + Math.random().toString(36).substr(2, 9)
        }
    }
}
</script>
